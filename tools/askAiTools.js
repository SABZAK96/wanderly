...
const search_places = {
  name: "search_places",
  description:
    "Searches Google Places for real venues matching a query, biased toward the trip's location — returns id, name, address, rating, price level, opening hours, and location for each match. Use this to find actual, addable candidates for an itinerary (it's the source of the placeId/location that add_activity_group/add_activity_solo require) — use web_search separately for pricing confirmation or opinion/review grounding, not for finding the venue itself. Phrase userQuery the way a manual search would, e.g. 'museums in Lisbon', not just 'museums'. lat/lng are the trip's own stored coordinates — never invent them. Pass nextToken only to fetch more results for the same userQuery/lat/lng after an earlier call returned one.",
  input_schema: {
    type: "object",
    properties: {
...
const list_activities = {
  ...
  description:
    "Returns all of the current user's own activities on this trip's calendar (activityName, date, startTime, endTime, address, placeId, location) — not other trip members' activities. Use when you need the user's schedule, especially before suggesting a packing list.",
  input_schema: {
    type: "object",
    properties: {},
    required: [],
